...
export class TareasCardComponent implements OnInit {
  ...
  constructor(public servicio: ServicioService, public dialog: MatDialog, public storage: AngularFireStorage) { }
  ...
  upload(titulo: String, comentarios: String, filepath: any, file: File, tarea_id: String) {
    ...
    task.snapshotChanges().pipe(last(), switchMap(() => fileRef.getDownloadURL()))
      .subscribe(url => {
        ...
        this.servicio.agregarTrabajo(new Trabajo(id, this.servicio.usuario.uid, titulo + "", "YYYY/MM/DD HH:MM:SS", tarea_id, url, comentarios + ""));
      })
